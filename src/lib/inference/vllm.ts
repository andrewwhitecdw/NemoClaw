...
function downloadModel(
  ...
  return new Promise((resolve) => {
    ...
    const tail: string[] = [];
    const outputStates = [
      { pending: "", stream: process.stdout },
      { pending: "", stream: process.stderr },
    ];
    const TAIL_MAX = 50;
    let resolved = false;
    ...
    function flushOutput(
      state: { pending: string; stream: NodeJS.WriteStream },
      flushAll = false,
    ): void {
      const end = flushAll
    ...
    function onChunk(buf: Buffer, state: { pending: string; stream: NodeJS.WriteStream }): void {
      lastOutputAt = Date.now();
      state.pending += buf.toString();
      flushOutput(state);
    }
    ...
    proc.on("error", (err: Error) => {
      for (const state of outputStates) flushOutput(state, true);
