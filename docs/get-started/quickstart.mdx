...
<AccordionGroup>
  ...
  <Accordion title="Installer Behavior and Platform Details">
    ...
    Authentication is optional for the public Nemotron 3 Ultra and DeepSeek V4 Flash recipes, but a read token reduces exposure to anonymous HTTP `429` rate limiting during these large downloads.
    Create a read token at [Hugging Face Access Tokens](https://huggingface.co/settings/tokens), export it before you launch the installer, and do not paste it into NemoClaw prompts or chat.
    ...
    The installer passes `HF_TOKEN` only to the temporary model downloader, and the long-running vLLM container does not retain it.
    If the download receives HTTP `429`, keep the existing cache, export `HF_TOKEN`, and run `nemoclaw onboard --resume`.
